...
def DRN_predict_evaluate(var_num, lead_time):
    ...
    for count in range(10):
        ...
        DRN_preds.append(DRN_model.predict([drn_X_test_lead_array_mask, drn_embedding_test_lead_array]))
    DRN_preds_average = np.mean(np.abs(DRN_preds), axis = 0)
    model_filename_preds = f"/Data/Delong_BA_Data/preds/DRN_preds_10_dummy/DRN_{var_names[var_num]}_lead_time_{lead_time}_dummy_preds.npy"
    ...
    model_filename = f"/Data/Delong_BA_Data/scores/DRN_scores_10_dummy/DRN_{var_names[var_num]}_lead_time_{lead_time}_scores.npy"
    np.save(model_filename, DRN_scores)
